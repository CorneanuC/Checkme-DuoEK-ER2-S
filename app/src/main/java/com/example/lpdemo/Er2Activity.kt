    ...
    private fun initView() {
        ...
        binding.savePdf.setOnClickListener {
            ...
            if (file != null) {
                ...
                Toast.makeText(this, "No data to generate PDF. Read files or start RT task first.", Toast.LENGTH_SHORT).show()
            }
        }
        ...
        LiveEventBus.get<InterfaceEvent>(InterfaceEvent.ER2.EventEr2ReadFileComplete)
            .observe(this) {
                ...
                if (data.fileName.contains("a")) {
                    ...
                    pdfGenerator.setEcgWaveData(ecgShorts, file.recordingTime, data.fileName, startTime)
                    ecgList.add(ecgData)
                    ecgAdapter.setNewInstance(ecgList)
                    ecgAdapter.notifyDataSetChanged()
                    // sampling rate：125HZ
