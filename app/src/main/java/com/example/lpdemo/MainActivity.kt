...
class MainActivity : AppCompatActivity(), BleChangeObserver {
    ...
    // Guard against duplicate EventEr2Info (BLE lib replays events on new BIOL subscriber)
    private var isNavigatingToDevice = false
    ...
    private fun initEventBus() {
        ...
        LiveEventBus.get<InterfaceEvent>(InterfaceEvent.ER2.EventEr2Info)
            .observe(this) {
                ...
                // ER2-S信心相联 定制版本
                Log.d(TAG, "EventEr2Info: branchCode=${data.branchCode}, model=${it.model}")
                if (isNavigatingToDevice) {
                    Log.d(TAG, "EventEr2Info: already navigated, ignoring duplicate event")
                    return@observe
                }
                isNavigatingToDevice = true
    ...
    override fun onResume() {
        super.onResume()
        // Reset navigation guard when user returns to MainActivity (e.g. presses back from device screen)
        isNavigatingToDevice = false
    }
